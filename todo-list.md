Cuando se habla de pixel entiendase a los tags de aplicaciones que miden interacciones de usuarios en un sitio, como Meta, RTB House, Criteo, Pinterest, Tiktok, etc

## Limpiar proyecto
Se va a dejar lo minimo necesario de la extension en el proyecto para crear una nueva.
- [x] Eliminar logica relacionada a Recargar pagina sin cache
- [x] Eliminar seccion de alertas y logica relacionada a alertas
- [x] Eliminar seccion de Digital DAta
...
### Sidepanel de Chrome
- [x] Agregar una Seccion por cada pixels configurado.
- [x] Los summary de las secciones deben incluir el numero de eventos registrados.

### Panel de configuracion
Es necesario un panel donde se pueda configurar los eventos para poder traducir los datos enviados a una tabla ordenada legible por humanos.
En este panel se tiene que poder crear, editar y eliminar pixels a medir.
Esta configuracion debe permitir configurar la traduccion de la url a la tabla con los campos que el usuario quiera usar. esto es independiente para cada pixel.
Ejemplo de url:
```
...
```
La interfaz implementada permite crear, editar y eliminar pixels. Cada pixel configura una coincidencia de URL y campos traducidos con parametro de origen, clave interna opcional y numero de decodificaciones. Los eventos se capturan por pestaña, se conservan hasta 100 llamadas y se muestran como tablas dentro de eventos colapsables.
